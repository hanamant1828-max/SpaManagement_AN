...
def accept_grouped_booking():
    ...
    try:
        ...
        if success_count > 0:
            print(f"✅ Successfully accepted {success_count} grouped bookings")
        ...
        if failure_count > 0:
            ...
            for failed in failed_bookings:
                error_details.append(f"❌ Cannot accept {failed['booking_label']}:\n{failed['error']}")

            print(f"⚠️ {failure_count} booking(s) failed validation:")
            for error_detail in error_details:
                print(f"  {error_detail}")

            return jsonify({
                'success': False,
                'message': f'Failed to accept {failure_count} booking(s) due to validation errors',
                'error_details': error_details,
                'errors': [r['error'] for r in failed_bookings]
            }), 400
