...
def get_customer_packages(customer_id):
    ...
    try:
        ...
        # PRIMARY: Get all active package assignments for this customer
        assignments = ServicePackageAssignment.query.filter_by(
            customer_id=customer_id,
            status='active'
        ).all()
        
        app.logger.info(f"Found {len(assignments)} active package assignments for customer {customer_id}")
