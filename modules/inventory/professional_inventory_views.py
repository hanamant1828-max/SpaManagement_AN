...
def inventory_pro():
    ...
    try:
        ...
        if location_filter:
            query = query.filter(SimpleInventoryItem.location == location_filter)
        ...
        if status_filter:
            if status_filter == 'low_stock':
                ...
                query = query.filter(SimpleInventoryItem.current_stock > SimpleInventoryItem.maximum_stock)
            elif status_filter == 'normal':
                query = query.filter(
                    and_(
                        SimpleInventoryItem.current_stock > SimpleInventoryItem.minimum_stock,
                        or_(
                            SimpleInventoryItem.maximum_stock.is_(None),
                            SimpleInventoryItem.current_stock <= SimpleInventoryItem.maximum_stock
                        )
                    )
                )
                ...
                query = query.filter(
                    and_(
                        SimpleInventoryItem.expiry_date.isnot(None),
                        SimpleInventoryItem.expiry_date < date.today()
                    )
                )
        ...
        if search_query:
            ...
            query = query.filter(
                or_(
                    SimpleInventoryItem.name.ilike(search_term),
                    SimpleInventoryItem.sku.ilike(search_term),
                    SimpleInventoryItem.barcode.ilike(search_term),
                    SimpleInventoryItem.description.ilike(search_term),
                    SimpleInventoryItem.supplier.ilike(search_term)
                )
            )
        ...
        items = query.order_by(
            ...
            SimpleInventoryItem.current_stock.asc(),
            SimpleInventoryItem.name.asc()
        ...
        locations = db.session.query(SimpleInventoryItem.location).distinct().filter(
            SimpleInventoryItem.location.isnot(None),
            SimpleInventoryItem.is_active == True
        ).all()
        ...
        recent_transactions = SimpleStockTransaction.query.join(SimpleInventoryItem).filter(
            SimpleInventoryItem.is_active == True
        ).order_by(desc(SimpleStockTransaction.date_time)).limit(50).all()
        
        # Get active alerts
        active_alerts = SimpleLowStockAlert.query.join(SimpleInventoryItem).filter(
            and_(
                SimpleInventoryItem.is_active == True,
...
def api_inventory_alerts():
    ...
    try:
        alerts = SimpleLowStockAlert.query.join(SimpleInventoryItem).filter(
            and_(
                SimpleInventoryItem.is_active == True,
                SimpleLowStockAlert.is_acknowledged == False,
                SimpleLowStockAlert.is_resolved == False
...
def add_professional_inventory_item():
    ...
    try:
        ...
        existing_item = SimpleInventoryItem.query.filter(
            or_(
                SimpleInventoryItem.sku == sku,
                and_(
                    SimpleInventoryItem.barcode == data.get('barcode'),
                    SimpleInventoryItem.barcode.isnot(None)
                )
            )
        ).first()
...
def api_quick_stock_update():
    ...
    try:
        ...
        item = SimpleInventoryItem.query.get_or_404(item_id)
